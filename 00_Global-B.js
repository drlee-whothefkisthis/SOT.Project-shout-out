...
(function () {
  ...
  const PACKAGE_THRESHOLD = 5; // ✅ 패키지 기준 장수
  ...
  // ✅ 이미 장바구니에 '기존에' 담겨있던(=갤러리에서 잠금 처리할) 키 스냅샷
...
/* =========================
  ...
  function toggleSelectByKey(key) {
    ...
    // ✅ 이미 장바구니에 '기존에' 담겨있던 사진이면 → 갤러리에서 잠금(선택/해제 불가)
    ...
    // ✅ 이번 갤러리에서 선택한(=로컬 선택) 상태면 → 해제(카트에서도 제거)
    if (localSelectedKeys.has(key)) {
      if (window.ShoutCart.has(photo._id)) window.ShoutCart.remove(photo._id);
  ...
  function hydrateLocalSelectedFromGlobal(){
    // ✅ '로컬 선택'을 전역에서 끌어오지 않음.
    //    대신, 페이지 진입 시점 localStorage 장바구니를 스냅샷으로 읽어 "잠금 대상"만 만든다.
    lockedCartKeys.clear();
    ...
    try {
      ...
      if (raw) {
        ...
        if (data && Array.isArray(data.items)) lists.push(data.items);
        if (data && Array.isArray(data.photos)) lists.push(data.photos);

        if (data && Array.isArray(data.events)) {
          for (const ev of data.events) {
            if (ev && Array.isArray(ev.items)) lists.push(ev.items);
            if (ev && Array.isArray(ev.photos)) lists.push(ev.photos);
          }
        }
        ...
        for (const list of lists) {
          for (const it of list) {
            const idRaw =
              (it && (it._id || it.photo_id || it.photoId || it.id)) ||
              (it && it.photo && (it.photo._id || it.photo.id)) ||
              (typeof it === "string" ? it : "");
            const id = String(idRaw || "").trim();
            if (id) lockedCartKeys.add(id);
          }
        }
      }
    } catch (e) {}

    // fallback: ShoutCart에서도 한번 더 읽어 합집합 (있으면)
    try {
  ...
  function syncUI(key) {
    ...
    if (card) {
      // cart-in(기존 담김)은 is-in-cart로 잠금. 로컬 선택은 is-selected.
      if (isLockedByKey(key)) {
  ...
  function createCardEl(photo, sizeClass) {
    ...
    // ✅ 기존 cart-in(이미 담김) 사진: 어두운 오버레이 + 완전 블락(호버/클릭/체크/선택 UI 없음)
    if (isLockedByKey(key)) {
      card.classList.add("is-in-cart");
      card.append(media); // badge 없음
      return card;
    }
...
/* =========================
  ...
  function syncModalCheckUI(key) {
    ...
    // ✅ cart-in(기존 담김) 잠금 상태면: 체크/선택 UI 표시 금지
    if (isLockedByKey(key)) {
  ...
  async function initGallery() {
    ...
    window.addEventListener("load", () => {
      ...
      // ✅ 잠금 대상(기존 cart-in) UI 반영
      for (const k of lockedCartKeys) { syncUI(k); }
    }, { once: true });
