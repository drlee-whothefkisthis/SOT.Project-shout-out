(function(){
  ...
  async function fetchDashboardSnapshot({ dataSource, snapshotType, periodKey, eventCode, tab, manualRefresh }) {
    ...
    const proxyMode = manualRefresh === true ? "dashboard_snapshot_refresh" : "dashboard_snapshot";
    const data = await fetchDashboardProxy(proxyMode, payload);
...
(function(){
  ...
  function renderWorkReport({ report, resolvedSections, editable }) {
    ...
    const readValue = value => escapeHtml(value === null || value === undefined || value === "" ? "—" : String(value));
    const inlinePair = (label, value, className = "") => `<span class="fr-history-inline-pair"><span>${escapeHtml(label)}</span><b class="${className}">${readValue(value)}</b></span>`;
    const operationClass = report.meta.operation_result === "부분 운영" ? "is-partial" : report.meta.operation_result === "운영 중단" ? "is-stopped" : "";
    const uploadClass = report.meta.upload_completion_status === "완료" ? "is-complete" : report.meta.upload_completion_status === "미업로드" ? "is-not-uploaded" : report.meta.upload_completion_status === "일부 완료" ? "is-partial" : "";
    const historyChecks = [
      ["업로드 완료", closingChecks.upload_completed],
      ["장비 반납 완료", closingChecks.equipment_returned],
      ["분실물 확인", closingChecks.lost_and_found_checked],
    ...
    return `<div class="fr-history-readonly">
      ...
      <article class="ctdash-card ctdash-section fr-section"><div class="ctdash-section-head"><div><div class="ctdash-kicker">1</div><h3>기본 정보</h3></div><span class="ctdash-tag">${escapeHtml(report.meta.weekday || "요일 미정")}</span></div>
        <div class="fr-history-basic">
          ...
          <div class="fr-history-basic-row"><span class="fr-history-row-label">운영 정보</span><div class="fr-history-inline-list">${inlinePair("날씨", report.meta.weather)}<i></i>${inlinePair("기온", report.meta.temperature)}<i></i>${inlinePair("운영 결과", report.meta.operation_result, `fr-history-status ${operationClass}`)}<i></i>${inlinePair("업로드", report.meta.upload_completion_status, `fr-history-status ${uploadClass}`)}</div></div>
          <div class="fr-history-basic-row"><span class="fr-history-row-label">작성 정보</span><div class="fr-history-inline-list">${inlinePair("작성자", report.meta.writer)}<i></i><span class="fr-history-inline-pair"><span>참여 스태프</span><b class="fr-history-staff-list">${participantStaff.length ? participantStaff.map(name => `<em>${escapeHtml(name)}</em>`).join("") : "—"}</b></span></div></div>
          ${report.meta.operation_result_reason ? `<div class="fr-history-basic-row"><span class="fr-history-row-label">운영 결과 사유</span><p class="fr-history-reason ${operationClass}">${readValue(report.meta.operation_result_reason)}</p></div>` : ""}
